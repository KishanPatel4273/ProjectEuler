...
\newline 
\newline
Let $(N_n, b_d)$ where $N_n \coloneq 10^n$, $b_d \coloneq 10^d$, $b_d < N_n$ and $n,d \in \mathbb{N}^+$
...
we can apply sequence lemma LS5 for $v|5$ else apply LS3. This will give
the $\frac{N!}{5^{z(N!)}} \mod b$. Then to get the solution $(N_n, b_d)$ 
we need to use sequence lemma LS4 to remove 2's for each 5 we removed. 
From LS4 we can remove $\lambda$ 2's at a time. However, then we would need
...
\begin{align}    
    S_5 &= \{5, 10, \cdots, b\} \\
    S_2 &= \{2, 4, \cdots , b\} \setminus S_5 \land |S_2| = Z \\
...
\begin{align}
    ...
    &5^{p_O} \leq \tau \\
\end{align}
These condition are not generally not going to be hold for $1 < p_O$, so
