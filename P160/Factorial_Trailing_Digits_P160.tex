...
we can apply sequence lemma LS3 for $v|5$ else apply LS2. This will give
the $\frac{N!}{5^{z(N!)}} \mod b$. Then to get the solution $(N_n, b_d)$ 
we need to use sequence lemma LS2 to remove 2's for each 5 we removed. 
From LS2 we can remove $\lambda$ 2's at a time. However, then we would need
...
\begin{align}
    ...
    R &= Z - z(N!)
\end{align}
There are $4\frac{b}{10}$ $p_v$ that can have LS2 applied to them. The following construction
